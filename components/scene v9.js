...
export class Scene {
    ...
    poseAngles(joint) {
        ...
        if (joint == this.neck) {
            ...
            const vec_bone = (new THREE.Vector3).subVectors(point_arm, point_articulation); //NE PAS TOUCHER
...
function setJointAnglesFromVects1(joint, vec_world, vec_child_world)
{
    ...
    // console.log("vec_child_local", vec_child_local);
    const vec_local = joint.parent.parent.clone().worldToLocal(vec_world.clone());
    // console.log("vec_local", vec_local);
...
function setJointAnglesFromVects5(joint, vec_child_world)
{
    ...
    var vec_local = normal.clone().applyMatrix3(normalMatrix.clone()).normalize()
