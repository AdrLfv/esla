...
function setJointAnglesFromVects1(joint, vec_world, vec_child_world)
{

    var vec_child_local = joint.parent.clone().worldToLocal(vec_child_world.clone());
    var vec_local = joint.parent.clone().worldToLocal(vec_world.clone());
...
function setJointAnglesFromVects5(joint, vec_child_world)
{
    ...
    var quat_pose_rot = new THREE.Quaternion();
    var vec_local = joint.position;
    quat_pose_rot.setFromUnitVectors(vec_local.clone().normalize(), vec_child_local.clone().normalize());
    joint.quaternion.rotateTowards(quat_pose_rot.clone(), 0.1);
    // joint.setRotationFromMatrix(joint.matrixWorld.clone().makeRotationFromQuaternion(quat_pose_rot));
}