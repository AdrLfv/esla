...
export class Scene {
    ...
    angleBetween2D(u, v) {
        ...
        // return Math.atan2(u.x, u.y) - Math.atan2(v.x, v.y);
    }

    test_angleBetween2D()
    {
        console.log(this.angleBetween2D(new THREE.Vector2(0,1), new THREE.Vector2(15,-15)));
        console.log(this.angleBetween2D(new THREE.Vector2(0,1), new THREE.Vector2(-15,15)));
    }
    ...
    poseAngles(joint) {
        ...
        const pose_spine_2 = ((new THREE.Vector3).copy(pose_right_shoulder)).add(pose_left_shoulder).multiplyScalar(0.5).sub(new THREE.Vector3(0,-1,0));
        ...
        if (joint == this.neck) {
            ...
            var point2 =  pose_right_shoulder; //((new THREE.Vector3).copy(neck)).add(aine).multiplyScalar(0.5);
    ...
    apply_rotation(joint) {
        ...
        if (joint == this.neck) {
            avatar_angles = this.anglesBetween3D(joint.children[0].position, joint.position, this.right_shoulder.position);
        ...
        // joint.rotation.x = lerp(joint.rotation.x, targeted_avatar_angle_x, 0.05);
        joint.rotation.y = lerp(joint.rotation.y,  targeted_avatar_angle_y, 0.05);
        // joint.rotation.z = lerp(joint.rotation.z, targeted_avatar_angle_z, 0.05);
